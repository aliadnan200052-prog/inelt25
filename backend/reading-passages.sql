/* ═══════════════════════════════════════════════════════════════════════
   ...
   It adds ONE column and FIVE functions. It does not touch start_exam,
...
/* ── 6 ─ Deleting a whole passage (admin only) ────────────────────────
   Removes every question carrying this passage, inactive ones included,
   so a passage taken out does not linger invisibly behind the list.

   There is no undo. The caller is expected to have been told how many
   questions are about to go and to have agreed; the count comes back so
   it can be checked against what was shown. */
create or replace function public.admin_delete_passage(p_key text)
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  return jsonb_build_object('deleted', v_rows);
end;
$$;
...
revoke all on function public.admin_delete_passage(text)           from public;
...
grant execute on function public.admin_delete_passage(text)          to authenticated;
